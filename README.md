...
| 解包 Unity 资源包，从里面直接取出 DLL | ❌ IL2CPP（`libil2cpp.so` 那种） |
...
- 认四种容器：**UnityFS**（Unity 5.3 以后）、**UnityWeb** / **UnityRaw**（更老的格式，第 6 版起内部结构与 UnityFS 一致）、**BuildPlayer-\***（打包玩家资源时的变体）
...
- **批量反编译**：选一个文件夹，把它里面所有 `.dll` / `.exe` 逐个扫出来，一次性全部导出成 `.cs`（对应 dnSpy 的「导出到项目」）
  - 扫描时按文件头确认是不是托管程序集，原生的 `.so` / 空壳 `.exe` 会跳过并告诉你跳了哪些
  - 每个程序集在目标目录下**单独占一个子目录**，不同 DLL 的同名命名空间不会互相覆盖
  - 所有程序集会在同一个暂存目录里解包，Unity 那套 `Assembly-CSharp.dll` 引用 `UnityEngine.dll` 的依赖关系能正常解析
  - 中途可取消，已经写出来的文件都是完整的
...
**想把一整个文件夹的 DLL 都反编译出来**：在主页点**反编译一整个文件夹**，选中游戏目录（或者只放 DLL 的目录），DroidSpy 会扫出里面所有托管程序集，确认后选一个保存位置，它会把每个 DLL 的源码分别整理进各自的子目录。
